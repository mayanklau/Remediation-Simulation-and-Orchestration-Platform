...
type GraphCanvasProps = {
  ...
  nodes: GraphNode[];
  edges: GraphEdge[];
  mode: "asset" | "attack" | "chain";
};

const nodeTypes = { enterprise: EnterpriseNode };

export function GraphCanvas({ title, description, nodes, edges, mode }: GraphCanvasProps) {
  ...
  return (
    <section className="panel graph-panel">
      ...
      <div className="graph-toolbar">
        <select value={kind} onChange={(event) => setKind(event.target.value)} aria-label="Filter node kind">
          {kinds.map((item) => <option key={item} value={item}>{item.replace("_", " ")}</option>)}
        </select>
        <select value={layout} onChange={(event) => setLayout(event.target.value as "layered" | "radial")} aria-label="Graph layout">
      ...
      <div className="flow-shell">
        <ReactFlow
          ...
          edges={flowEdges}
          nodeTypes={nodeTypes}
          defaultViewport={{ x: 28, y: 150, zoom: mode === "attack" ? 0.92 : 0.82 }}
          minZoom={0.45}
          maxZoom={1.6}
          onNodeClick={(_, node) => setSelected(node.data.raw as GraphNode)}
        >
