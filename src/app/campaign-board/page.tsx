...
export default async function CampaignBoardPage() {
  ...
  if ((await prisma.remediationCampaign.count({ where: { tenantId: tenant.id } })) === 0) {
    await createOrRefreshCampaign(tenant.id, {
      ...
      objective: "Reduce critical and internet-exposed risk through simullation-backed remediation waves.",
      owner: "security-lead",
  ...
  return (
    <>
      ...
      >
        <ApiButton path="/api/campaigns" label="Refresh campaign" payload={{ name: "Critical exposure reduction", objective: "Reduce critical and internet-exposed risk through simullation-backed remediation waves.", owner: "security-lead", criteria: { minRiskScore: 70 } }} />
      </PageHeader>
