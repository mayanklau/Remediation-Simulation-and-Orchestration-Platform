    ...
    <>
      ...
      <section className="grid cols-2" style={{ marginTop: 16 }}>
        <div className="panel">
          ...
          <table className="table">
            <thead>
              <tr><th>Family</th><th>Findings</th><th>Mapping</th><th>Exploit</th><th>Remediation</th></tr>
            </thead>
            <tbody>
              {analytics.scannerCoverage.map((coverage) => (
                <tr key={coverage.family}>
                  <td><strong>{coverage.family}</strong><div className="muted">{coverage.readyForAttackGraph ? "graph ready" : "needs more mapping"}</div></td>
      ...
      <section className="panel">
        ...
        <div className="attack-graph-board">
          <div className="graph-column"><span>Entry</span>{analytics.graph.nodes.filter((node) => node.kind === "entry").slice(0, 5).map((node) => <GraphNode key={node.id} node={node} />)}</div>
          <div className="graph-column wide"><span>Reachability and exploit edges</span>{analytics.graph.edges.slice(0, 10).map((edge) => <div className={`graph-link ${edge.relation}`} key={edge.id}><strong>{labelFor(edge.from, analytics.graph.nodes)}</strong><span>{edge.label}</span><strong>{labelFor(edge.to, analytics.graph.nodes)}</strong></div>)}</div>
          <div className="graph-column"><span>Targets and breakers</span>{analytics.graph.nodes.filter((node) => node.kind === "crown_jewel" || node.kind === "breaker").slice(0, 6).map((node) => <GraphNode key={node.id} node={node} />)}</div>
        </div>
      </section>

      <section className="panel">
        <div className="stack-head"><div><h2>Vulnerability Chaining Graph</h2><p>Ordered exploit chain view with scanner source, technique, difficulty, residual risk, and the control that breaks the path.</p></div><StatusBadge value={`${analytics.summary.vulnerabilityChains} chains`} /></div>
        <div className="chain-grid">
          {analytics.vulnerabilityChainGraph.map((chain) => <article className="chain-card" key={chain.pathId}><div className="chain-head"><div><strong>{chain.pathName}</strong><span>{chain.beforeRemediationRisk}% before / {chain.afterRemediationRisk}% after</span></div><StatusBadge value={chain.difficulty} /></div><div className="chain-rail">{chain.nodes.map((node, index) => <div className="chain-node-wrap" key={`${chain.pathId}-${node.id}-${index}`}><GraphNode node={node} compact />{index < chain.nodes.length - 1 && <div className="chain-arrow">risk transfer</div>}</div>)}</div></article>)}
        </div>
      </section>
      ...
      <section className="panel">
        ...
        <table className="table">
          <thead><tr><th>Path</th><th>Difficulty</th><th>Before</th><th>After</th><th>Delta</th><th>Scanners</th><th>Evidence</th><th>Priority</th></tr></thead>
          <tbody>
            {analytics.paths.map((path) => <tr key={path.id}><td><strong>{path.name}</strong><div>{path.hops.join(" -> ")}</div><div>{path.chain.map((step) => step.technique).join(" -> ")}</div></td><td><StatusBadge value={path.difficulty} /></td><td>{path.beforeRemediationRisk}%</td><td>{path.afterRemediationRisk}%</td><td>{path.riskDelta}%</td><td>{path.scannerInputs.join(", ")}</td><td>{path.evidenceRequirements.slice(0, 3).join(", ")}</td><td><StatusBadge value={path.remediationPriority} /></td></tr>)}
          </tbody>
        </table>
      </section>
    </>
...
function GraphNode({ node, compact = false }: { node: { label: string; kind: string; group: string; risk: number }; compact?: boolean }) {
  return <div className={`graph-node ${node.kind} ${compact ? "compact" : ""}`}><small>{node.kind.replace("_", " ")}</small><strong>{node.label}</strong><span>{node.group} | {node.risk}%</span></div>;
}
